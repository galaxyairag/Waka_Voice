...
# ⚡ OPTIMISATION: Cache global pour éviter les imports répétés (fix timeout 502/connection reset)
_tools_cache = None
_tools_modules_loaded = False
...
def get_tools_definition():
    ...
    Utilise un cache global pour éviter les imports répétés (fix Azure timeout).
    ...
    # ⚡ Retourner le cache s'il existe (évite les imports répétés)
    if _tools_cache is not None:
        return _tools_cache

    # Import lazy: charger les modules seulement la première fois
...
def warmup_tools():
    """
    Pré-charge tous les modules tools pour éviter le timeout au premier appel.
    À appeler au démarrage de l'application.
    """
    global _tools_modules_loaded
    if _tools_modules_loaded:
        return True

    try:
        # Pré-charger les définitions
        get_tools_definition()
        _tools_modules_loaded = True
        return True
    except Exception as e:
        print(f"⚠️ Erreur warmup tools: {e}")
        return False


# Export des fonctions principales
__all__ = ['get_tools_definition', 'execute_tool', 'warmup_tools']
